...
function BoardPage() {
  ...
  useEffect(() => {
    if (!showMap) return;
    const params = new URLSearchParams({ postType: "제안", hasLocation: "true", limit: "500" });
    if (city) params.set("city", city);
    fetch(`/api/proposals?${params}`)
      .then(r => r.json())
  ...
  return (
    <div className="max-w-3xl mx-auto px-2 pb-12">
      ...
      <div className="flex items-center gap-2 py-4 border-b border-border mb-0">
        ...
        <h1 className="text-lg font-black text-foreground">💡 공약제안 게시판</h1>
            ...
            <button key={c}
              ...
              className={`px-2.5 py-1 rounded-full text-xs font-semibold transition-colors ${
                (c === "전체" && !city) || c === city
                  ...
                  : "bg-background border border-border text-muted hover:border-amber-300 hover:text-amber-600"
              }`}
            >{c}</button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 relative">
            <input
              ...
              className="w-full text-xs rounded-lg border border-border bg-background px-3 py-1.5 pr-7 focus:outline-none focus:border-amber-400"
            />
            {searchInput && (
            ...
            {(["latest", "popular"] as const).map(s => (
              <button key={s}
                ...
                className={`px-2.5 py-1.5 font-semibold transition-colors ${
                  sort === s ? "bg-amber-500 text-white" : "bg-background text-muted hover:bg-amber-50"
                }`}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-1 py-4 flex-wrap">
          <button
            ...
            disabled={page === 1}
            className="px-2.5 py-1.5 text-xs rounded border border-border text-muted hover:border-amber-400 hover:text-amber-600 disabled:opacity-30 disabled:pointer-events-none"
          >‹ 이전</button>
              ...
              <button key={p}
                ...
                className={`w-8 h-8 text-xs rounded border font-semibold transition-colors ${
                  p === page
                    ...
                    : "border-border text-muted hover:border-amber-400 hover:text-amber-600"
                }`}
              >{p}</button>
            );
          })}
      ...
      <div className="flex justify-center pt-2">
        <Link href="/proposals?type=suggestion"
          className="px-5 py-2.5 bg-amber-500 text-white text-sm font-bold rounded-xl hover:bg-amber-600 transition-colors">
          💡 공약 제안하기
        </Link>
      </div>
    </div>
...
export default function SuggestionsPage() {
  return (
    <Suspense fallback={<div className="flex justify-center py-20"><div className="w-6 h-6 border-2 border-amber-400 border-t-transparent rounded-full animate-spin" /></div>}>
      <BoardPage />
    </Suspense>
  );
}
