...
function generatePinSVG(emoji: string, color: string): string {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 44 44"><circle cx="22" cy="22" r="20" fill="${color}" stroke="white" stroke-width="2.5"/><text x="22" y="29" font-size="22" text-anchor="middle">${emoji}</text></svg>`;
...
export default function NaverMap({ pledges, onPledgeClick }: NaverMapProps) {
  ...
  // Pin appearance — fetched from admin settings on mount
  ...
  const addPledgeMarkers = useCallback(
    (map: naver.maps.Map) => {
      ...
      const pinUrl = generatePinSVG(pinSettings.emoji, pinSettings.color);
