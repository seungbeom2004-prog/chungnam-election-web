    ...
    for (const [, group] of groups) {
      ...
      const content = isMultiple
        ? // Stack badge for multiple items at same coordinate
          ...
          `<div style="width:22px;height:22px;border-radius:50%;background:${color};border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;">` +
          `<span style="font-size:9px;font-weight:800;color:${first.postType === "민원" ? "white" : "#1F2937"};">${group.length}</span>` +
          `</div></div>`
        : // Single dot
          `<div style="width:14px;height:14px;border-radius:50%;background:${color};border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,0.3);cursor:pointer;"></div>`;
